...
<body class="bg-gray-100 text-gray-900">
    ...
    <header class="bg-white shadow p-6 text-center">
        ...
        <nav class="mt-4 space-x-6">
            <a href="{{ route('actualites.index') }}" class="text-black hover:text-green-600  font-sans uppercase">ACTUALITÉS</a>
            <a href="{{ route('transferts.index') }}" class="text-black hover:text-green-600  font-sans uppercase">TRANSFERTS</a>
            <a href="{{ route('championsleague.index') }}" class="text-green-600 font-sans uppercase font-bold">LIGUE DES CHAMPIONS</a>
    ...
    <main class="max-w-4xl mx-auto mt-10 p-6 bg-white rounded shadow">
        <h1 class="text-3xl font-bold text-black-600 mb-6">Détails du match</h1>

        @if ($champions->image)
            <img src="{{ $champions->image }}" alt="Image du transfert" class="w-full h-full object-cover mb-4 rounded">
        @endif

        <div class="flex flex-col gap-4 text-lg">
            <p><span class="font-semibold">Titre :</span> {{ $champions->titre }}</p>
            <p><span class="font-semibold">contenu :</span> {!! $champions->contenu !!}</p>
            <p><span class="font-semibold">Date:</span> {{ $champions->date }}</p>
        </div>
        <div class="mt-8">
            <a href="{{ route('actualites.index') }}" class="text-black-600 hover:underline text-sm">← Retour à l'accueil</a>
        </div>
        <div class="mt-8">
            <a href="{{ route('championsleague.index') }}" class="text-black-600 hover:underline text-sm">← Retour aux actualités de la League des Champions</a>
        </div>
    </main>
    ...
    <footer class="bg-white shadow mt-16">
        <div class="max-w-4xl mx-auto px-6 py-10 text-center text-gray-700 flex flex-col items-center">
            ...
            <div class="flex flex-wrap justify-center gap-6 text-sm mb-6">
                ...
                <a href="{{ route('championsleague.index') }}" class="hover:underline font-bold">Ligue des Champions</a>
