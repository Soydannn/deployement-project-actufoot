        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach ($transferts as $transfert)
                <article class="border border-gray-200 rounded p-4 shadow hover:shadow-lg transition flex flex-col">
                    ...
                    <a href="{{ route('transferts.show', ['id' => $transfert->id]) }}" class="text-green-600 hover:underline px-4 py-2 rounded">En savoir plus</a>
                </article>
            @endforeach
        </div>
    @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach ($champions as $champion)
                <article class="border border-gray-200 rounded p-4 shadow hover:shadow-lg transition">
                    ...
                    <a href="{{ route('championsleague.show', ['id' => $champion->id]) }}" class="text-green-600 hover:underline px-4 py-2 rounded">En savoir plus</a>
                </article>
            @endforeach
        </div>
    @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach ($nations as $nation)
                <article class="border border-gray-200 rounded p-4 shadow hover:shadow-lg transition">
                    ...
                    <a href="{{ route('nationsleague.show', ['id' => $nation->id]) }}" class="text-green-600 hover:underline px-4 py-2 rounded">En savoir plus</a>
                </article>
            @endforeach
        </div>
    @endif
...
<section class="mb-12 max-w-6xl mx-auto px-4 py-8 bg-white rounded shadow">
    ...
    @endif
        ...
        <div class="flex justify-between mt-8">
            <a href="{{ route('actualites.index') }}" class="text-black hover:underline">← Retour à la page d'actualités</a>
        </div>
    </main>
